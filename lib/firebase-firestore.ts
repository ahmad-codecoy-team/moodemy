...
import type { Content, CreateContentInput, UpdateContentInput, UserProfile, MoodEntry } from '@/types';
...
export class FirebaseFirestoreService {
  ...
  static async getContentById(id: string): Promise<Content | null> {
    try {
      const contentDocs = [
        'home_screen', 'splash_screen', 'signup_message',
        'login_message', 'about_us', 'help_support', 'privacy_policy'
      ];
  ...
  static async createContent(contentData: CreateContentInput): Promise<Content> {
    ...
    throw new Error('Cannot create new content documents. Only predefined content types can be updated.');
  }

  // Update content by internal ID
  static async updateContent(id: string, updates: UpdateContentInput): Promise<Content> {
    try {
  ...
  static async updateContentByDocName(docName: string, updates: UpdateContentInput): Promise<Content> {
    try {
      const updateData = {
        ...updates,
        updatedAt: new Date().toISOString(),
  ...
  static async createUserProfile(uid: string, profileData: UserProfile): Promise<void> {
    try {
  ...
  // Create mood entry
  static async createMoodEntry(moodData: Omit<MoodEntry, 'id'>): Promise<MoodEntry> {
    try {
